...
# Configuration paths
MODEL_PATH = 'Baseline_CNN/baseline_cnn_embeddings.h5'
EMBEDDINGS_PATH = 'Baseline_CNN/saved_embeddings.pkl'
REGISTERED_IMAGES_PATH = 'Baseline_CNN/registered_images.txt'
...
def preprocess_face(face_img):
    face_img = cv2.resize(face_img, (100, 100))  # Match model input size
    face_img = cv2.cvtColor(face_img, cv2.COLOR_BGR2RGB)
    face_img = (face_img - face_img.mean()) / face_img.std()
    return np.expand_dims(face_img, axis=0)

def get_embedding(face_img):
    embedding = baseline_model.predict(preprocess_face(face_img))[0]
